...
const AdminDashboard = ({ onViewBooking }) => {
  const { 
    ...
    updateBookingStatus, assignAgent, deleteBooking, updateBooking, loading 
  } = useApp();
                  ...
                  {bookings.length === 0 ? (
                    ...
                    bookings.map((booking) => (
                      <tr key={booking.id} className="hover:bg-slate-900/35 transition group">
                        ...
                        {/* Actions */}
                        <td className="py-4 text-right flex items-center justify-end gap-1.5 font-medium">
                          <button
                            onClick={() => openStatusModal(booking)}
                            className="bg-slate-800 hover:bg-slate-700 text-white font-semibold py-1 px-2.5 rounded text-[10px] border border-slate-750 transition cursor-pointer"
                          >
                            Update
                          </button>
                          <button
                            onClick={() => openEditModal(booking)}
                            className="bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white font-semibold p-1.5 rounded text-[10px] border border-slate-750 transition cursor-pointer"
                            title="Edit Booking"
                          >
                          ...
                          <button
                            ...
                            className="bg-red-950/40 hover:bg-red-900/60 text-red-400 font-semibold p-1.5 rounded text-[10px] border border-red-900/30 hover:border-red-800 transition cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in-up">
          <div className="glass-card max-w-sm w-full p-6 rounded-2xl border border-slate-800 shadow-2xl">
            ...
            <p className="text-xs text-slate-400 mb-4">
              Are you sure you want to permanently delete booking <span className="text-red-400 font-mono font-bold">{selectedBooking.booking_code}</span>?
            </p>
      ...
      {showEditModal && selectedBooking && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto animate-fade-in-up py-8">
          <div className="glass-card max-w-lg w-full p-6 rounded-2xl border border-slate-800 shadow-2xl relative">
            ...
            <p className="text-xs text-slate-400 mb-4">Modify customer and visit logistics for <span className="text-emerald-400 font-mono font-bold">{selectedBooking.booking_code}</span>.</p>
